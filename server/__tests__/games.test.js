...
describe('POST /api/games', () => {
  ...
  test('Player1 Creating offline game', async () => {
    const { body } = await api
      .post(baseUri)
      .send(gameOffline)
      .set('Authorization', `Bearer ${player1Token}`)

    expect(body.id).toEqual(gameOffline.id)
    expect(body.type).toEqual(gameOffline.type)
    expect(body.player1).toEqual(player1.id)
    expect(body.player2).toEqual(null)
  })

  test('When an Offline game ends in a Tie', async () => {
    await Game.update(
      { moves: defaultTieMoves },
      {
        where: {
    ...
    await api
      .post(`${baseUriOffline}/${gameOffline.id}`)
      .send({ move: [3, 3] })
      .set('Authorization', `Bearer ${player1Token}`)

    const game = await Game.findByPk(gameOffline.id)
    expect(game.isFinished).toEqual(true)
    expect(game.winner).toEqual(null)
  })

  test('player 1 leaderboard after offline game n1', async () => {
    const player1LeaderBoard = await Leaderboard.findOne({
      where: {
        userId: player1.id,
      },
  ...
  test('Reset offline game state', async () => {
    await Game.update(
      { isFinished: false, moves: [], winner: null, inTurn: player1.id },
      {
        where: {
  ...
  test('When Player1 wins the offline game', async () => {
    for (let i = 0; i < player1Moves.length; i++) {
      // Player 1 move
      await api
        .post(`${baseUriOffline}/${gameOffline.id}`)
  ...
  test('player 1 leaderboard after offline game n2', async () => {
    const player1LeaderBoard = await Leaderboard.findOne({
      where: {
        userId: player1.id,
      },
